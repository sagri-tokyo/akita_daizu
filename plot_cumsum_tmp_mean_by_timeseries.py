...
def main():
    ...
    point2_by_date = point_2.groupby([point_2['datetime'].dt.date])[target_col].agg(groupby_idx)
    ...
    point2_by_date['cumsum_tmp_mean'] = point2_by_date['tmp_mean'].cumsum()

    target_date_range = point1_by_date.index

    point_1_tmp_mean = xr.open_dataset('downloads/tables/point_1_TMP_mean.nc')
    point_2_tmp_mean = xr.open_dataset('downloads/tables/point_2_TMP_mean.nc')
    ...
    point_2_df_tmp_mean = point_2_tmp_mean.sel(lat=point_2_lat, lon=point_2_lon, method="nearest").to_dataframe()
    point_1_df_tmp_mean = point_1_df_tmp_mean.loc[target_date_range]
    point_2_df_tmp_mean = point_2_df_tmp_mean.loc[target_date_range]
    point_1_df_tmp_mean['cumsum_tmp_mean'] = point_1_df_tmp_mean[target_index_1].cumsum()
    point_2_df_tmp_mean['cumsum_tmp_mean'] = point_2_df_tmp_mean[target_index_1].cumsum()
    ...
    idx_name = 'cumsum_tmp_mean'
    name = f'{idx_name}_timeseries_tokunaga'
    filename = f'plot_{name}.jpg'
    save_figure_path = figure_path / filename
    ax = sns.lineplot(x='datetime', y=idx_name, data=point2_by_date)
    sns.lineplot(x='time', y=idx_name, data=point_2_df_tmp_mean)
    diff = point2_by_date['cumsum_tmp_mean'] - point_2_df_tmp_mean['cumsum_tmp_mean']

    _diff = round(diff[diff.index == pd.to_datetime('2021-08-17')].iloc[0], 3)
    label = f'出穂日までの積算気温差分: {_diff}[℃]'
    ax.text(point2_by_date.index.min(), point2_by_date[idx_name].max(), label, fontsize=15)
    plt.xlim(point2_by_date.index.min(), pd.to_datetime('2021-08-17'))
    plt.title('計測地点: 徳永郷')
    plt.ylabel('積算気温')
    plt.legend(['MIHARAS観測値', 'メッシュ農業気象データ'])
    plt.savefig(save_figure_path)
